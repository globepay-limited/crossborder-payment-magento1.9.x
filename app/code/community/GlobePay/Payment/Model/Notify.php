<?php
class Globepay_Payment_Model_Notify
{
    ...
    public function handleNotify(array $request)
    {
        try {
            ...
            if(!$object){
               Mage::helper('globepay')->log ( " invalid result:" .$json);
               echo 'fali';
               return;
            }
            ...
            if($sign!=$object->sign){
               Mage::helper('globepay')->log ( " invalid sign:" .$json);
               echo 'fali';
               return;
            }
            ...
            if(count($order_ids)!=2){
                Mage::helper('globepay')->log ( " invalid order id:" .$json);
                echo 'fali';
                return;
            }
            ...
            if(!$resArr||$resArr->result_code!='PAY_SUCCESS'){
               Mage::helper('globepay')->log ( " order callback fail:" .$result);
                echo 'fail';
                exit;
            }
            ...
            ->setPreparedMessage(Mage::helper('globepay')->__('transactionId: "%s".', $transaction_id))
