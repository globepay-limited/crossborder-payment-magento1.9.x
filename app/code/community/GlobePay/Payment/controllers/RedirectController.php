<?php
class Globepay_Payment_RedirectController extends Mage_Core_Controller_Front_Action {
    ...
    public function indexAction() {
        ...
        if ($orderId) {
            ...
            $order = Mage::helper('globepay')->getOrder();
        }
    	...
    	$callback = Mage::getUrl('globepay/redirect/callback',array('id' =>$new_id));
    	
    	$partner_code =Mage::helper('globepay')->getConfigData('mch_id');
    	$credential_code = Mage::helper('globepay')->getConfigData('mch_secret');
    	...
    	$channel=null;
    	if(Mage::helper('globepay')->isWeixinClient ()){
    	    $url ="https://pay.globepay.co/api/v1.0/wechat_jsapi_gateway/partners/$partner_code/orders/$new_id";
    	    
    	}else{
    	    
    	    if(Mage::helper('globepay')->is_app_client()&&(1==Mage::helper('globepay')->getConfigData('enable_h5'))){
    	        $url ="https://pay.globepay.co/api/v1.0/h5_payment/partners/$partner_code/orders/$new_id";
    	        $channel = 'Wechat';
    	    }else{
    	        $url ="https://pay.globepay.co/api/v1.0/gateway/partners/$partner_code/orders/$new_id";
    	    }
    	...
    	$data->notify_url= Mage::getUrl('globepay/notify');
    	if($channel){
    	...
    	try {
    	    ...
    	    if(!$resArr){
    	        Mage::helper('globepay')->log ( print_r($resArr,true));
    	        throw new Exception(Mage::helper('globepay')->__('This request has been rejected by the remote service!'));
    	    }
    	
    	    if(!isset($resArr->result_code)||$resArr->result_code!='SUCCESS'){
    	        Mage::helper('globepay')->log ( print_r($resArr,true));
    	        throw new Exception(Mage::helper('globepay')->__(sprintf('ERROR CODE:%s',$resArr->result_code.$resArr->return_msg)));
    	    }
    	    	
    	    if(!Mage::helper('globepay')->isWeixinClient ()&&!(Mage::helper('globepay')->is_app_client()&&(1==Mage::helper('globepay')->getConfigData('enable_h5')))){
    	        if(Mage::helper('globepay')->getConfigData('qrcode_location')=='local'){
    	    ...
    	    Mage::helper('globepay')->log ( $e->getMessage());
    	    echo $e->getMessage();
    	    return;
    	}
    }
       ...
       <!DOCTYPE html>
            ...
            <body>
             <div class="wx-title">微信支付收银台</div>
              ...
              <div class="left">
                 <div class="qrcon">
                   <h5><img src="<?php print  Mage::getDesign()->getSkinUrl('images/globepay/globepay.png');?>" alt=""></h5>
                     ...
                     <?php if(!Mage::helper('globepay')->isWeixinClient()&&!(Mage::helper('globepay')->is_app_client())){ ?>
                            请使用微信"扫一扫"<br/>
            				扫描二维码支付
                     <?php }else{?>
                            请截图保存此支付二维码<br/>
             ...
             <div class="sys"><img src="<?php print  Mage::getDesign()->getSkinUrl('images/globepay/wechat-sys.png');?>" alt=""></div>
          </div>
          <script src="<?php print  Mage::getDesign()->getSkinUrl('js/globepay/jquery-2.1.4.js');?>"></script>
             ...
             (function($){
            		window.view={
        				query:function () {
        			        $.ajax({
        			            type: "POST",
        			            url: '<?php echo Mage::getUrl('globepay/redirect/query',array('id' => $new_id))?>',
        			            timeout:60000,
        			            cache:false,
        			            dataType:'json',
    ...
    private function checkout_success($resArr,$order,$type){
        $globepay_order_id= $resArr->partner_order_id;

        $payment = $order->getPayment();
        $payment->setTransactionId($globepay_order_id)
            ->setCurrencyCode() // $request['currency']
            ->setPreparedMessage(Mage::helper('globepayalipay')->__('transactionId: "%s".', $globepay_order_id))
        ...
        if ($invoice && ! $order->getEmailSent()) {
            ...
            ->addStatusHistoryComment(Mage::helper('globepay')->__('Notified customer about invoice #%s.', $invoice->getIncrementId()))
