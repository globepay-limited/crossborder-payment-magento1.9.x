<?php
class Globepayalipay_Payment_RedirectController extends Mage_Core_Controller_Front_Action {
    ...
    public function indexAction() {
        ...
        if ($orderId) {
            ...
            $order = Mage::helper('globepayalipay')->getOrder();
        }
    	...
    	$callback = Mage::getUrl('globepayalipay/redirect/callback',array('id' =>$new_id));
    	
    	$partner_code =Mage::helper('globepayalipay')->getConfigData('mch_id');
    	$credential_code = Mage::helper('globepayalipay')->getConfigData('mch_secret');
    	...
    	$channel=null;
    	if(Mage::helper('globepayalipay')->is_app_client()){
    	    $url ="https://pay.globepay.co/api/v1.0/h5_payment/partners/$partner_code/orders/$new_id";
    	    $channel = 'Alipay';
    	}else{
    	    $url ="https://pay.globepay.co/api/v1.0/alipay/partners/$partner_code/orders/$new_id";
    	}
		...
		$data->notify_url= Mage::getUrl('globepayalipay/notify');
    	...
    	try {
    	    ...
    	    if(!$resArr){
    	        Mage::helper('globepayalipay')->log ( print_r($resArr,true));
    	        throw new Exception(Mage::helper('globepayalipay')->__('This request has been rejected by the remote service!'));
    	    }
    	
    	    if(!isset($resArr->result_code)||$resArr->result_code!='SUCCESS'){
    	        Mage::helper('globepayalipay')->log ( print_r($resArr,true));
    	        throw new Exception(Mage::helper('globepayalipay')->__(sprintf('ERROR CODE:%s',$resArr->result_code.$resArr->return_msg)));
    	    }
			...
			if(Mage::helper('globepayalipay')->isWeixinClient()){
    	    ...
    	    Mage::helper('globepayalipay')->log ( $e->getMessage());
    	    echo $e->getMessage();
    	    return;
    	}
    }
	...
	private function CopyLink($url,$order,$subject,$new_id){
        ...
        ?>
            ...
            <style>
             ...
             <script src="<?php print  Mage::getDesign()->getSkinUrl('js/globepay/clipboard.js');?>"></script>
            </head>
            ...
            <body>
	            <div class="wx-title">支付宝</div>
	              <div class="qrbox clearfix">
		              <div class="left">
		                 <div class="qrcon">
		                   <h5><img src="<?php print  Mage::getDesign()->getSkinUrl('images/globepayalipay/alipay.png');?>" alt=""></h5>
	             ...
	             (function($){
	            		window.view={
	        				query:function () {
	        			        $.ajax({
	        			            type: "POST",
	        			            url: '<?php echo Mage::getUrl('globepayalipay/redirect/query',array('id' => $new_id))?>',
	        			            timeout:60000,
	        			            cache:false,
	        			            dataType:'json',
	...
	private function checkout_success($resArr,$order,$type){
		...
		if ($invoice && ! $order->getEmailSent()) {
			...
			->addStatusHistoryComment(Mage::helper('globepayalipay')->__('Notified customer about invoice #%s.', $invoice->getIncrementId()))
