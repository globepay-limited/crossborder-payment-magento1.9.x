<?php
class Globepayalipay_Payment_Model_Notify
{
    ...
    public function handleNotify(array $request)
    {
        try {
            ...
            if(!$object){
               Mage::helper('globepayalipay')->log ( " invalid result:" .$json);
               echo 'fali';
               return;
            }
            ...
            if($sign!=$object->sign){
               Mage::helper('globepayalipay')->log ( " invalid sign:" .$json);
               echo 'fali';
               return;
            }
            ...
            if(count($order_ids)!=2){
                Mage::helper('globepayalipay')->log ( " invalid order id:" .$json);
                echo 'fali';
                return;
            }
            ...
            $url="https://pay.globepay.co/api/v1.0/gateway/partners/$partner_code/orders/{$object->partner_order_id}";
            ...
            if(!$resArr||$resArr->result_code!='PAY_SUCCESS'){
               Mage::helper('globepayalipay')->log ( " order callback fail:" .$result);
                echo 'fail';
                exit;
            }
